...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
   ...
   sprintf(str, "****** WIFI Web Server demonstration****** \n\n");
...
static int wifi_start(void)
{
 ...
 /*Initialize and use WIFI module */
  ...
  {
    sprintf(str,("ES-WIFI Initialized.\n"));
    HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
    if(WIFI_GetMAC_Address(MAC_Addr) == WIFI_STATUS_OK)
    {
      sprintf(str,("> eS-WiFi module MAC Address : %02X:%02X:%02X:%02X:%02X:%02X\n",
               MAC_Addr[0],
               MAC_Addr[1],
               MAC_Addr[2],
               MAC_Addr[3],
    ...
    {
      sprintf(str,("> ERROR : CANNOT get MAC address\n"));
      HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
      return -1;
    }
  }
...
int wifi_connect(void)
{
  ...
  if( WIFI_Connect(SSID, PASSWORD, WIFI_ECN_WPA2_PSK) == WIFI_STATUS_OK)
  {
    ...
    {
      sprintf(str,("> es-wifi module connected: got IP Address : %d.%d.%d.%d\n",
               IP_Addr[0],
               IP_Addr[1],
               IP_Addr[2],
               IP_Addr[3]));
    ...
    {
		  sprintf(str,(" ERROR : es-wifi module CANNOT get IP address\n"));
		  HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
      return -1;
    }
  }
  ...
  {
		 sprintf(str,("ERROR : es-wifi module NOT connected\n"));
		   HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
     return -1;
  }
  return 0;
...
int wifi_server(void)
{
  ...
  {
    sprintf(str,("ERROR: Cannot start server.\n"));
    HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
  }

  sprintf(str,("Server is running and waiting for an HTTP  Client connection to %d.%d.%d.%d\n",IP_Addr[0],IP_Addr[1],IP_Addr[2],IP_Addr[3]));
  HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
  do
  {
    uint8_t RemoteIP[4];
    ...
    {
        sprintf(str,("Waiting connection to  %d.%d.%d.%d\n",IP_Addr[0],IP_Addr[1],IP_Addr[2],IP_Addr[3]));
        HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
    }

    sprintf(str,("Client connected %d.%d.%d.%d:%d\n",RemoteIP[0],RemoteIP[1],RemoteIP[2],RemoteIP[3],RemotePort));
    HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
    ...
    {
      sprintf(str,("ERROR: failed to close current Server connection\n"));
      HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
      return -1;
    }
  }
  ...
  {
    sprintf(str,("ERROR: Cannot stop server.\n"));
    HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
  }

  sprintf(str,("Server is stop\n"));
  HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
  return 0;
}
...
static bool WebServerProcess(void)
{
  ...
  {
   sprintf(str,("get %d byte from server\n",respLen));
   HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
   ...
   if( respLen > 0)
   {
      if(strstr((char *)resp, "GET")) /* GET: put web page */
      {
        ...
        {
          sprintf(str,("> ERROR : Cannot send web page\n"));
          HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
        }
        else
        {
          sprintf(str,("Send page after  GET command\n"));
          HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
        }
       }
       else if(strstr((char *)resp, "POST"))/* POST: received info */
       {
         sprintf(str,("Post request\n"));
         HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
         ...
         {
           sprintf(str,("Send Page after POST command\n"));
           HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
         }
       }
     }
  ...
  {
    sprintf(str,("Client close connection\n"));
    HAL_UART_Transmit(&huart1, (uint8_t*) str, (uint16_t) strlen(str), 1000);
  }
  return stopserver;
...
/**
  ...
  * @retval None
  */
static WIFI_Status_t SendWebPage(uint8_t ledIsOn, uint8_t data)
{
  uint8_t  data_[50];
  ...
  sprintf((char *)data_, "%d", data);
  strcat((char *)http, (char *)data_);
